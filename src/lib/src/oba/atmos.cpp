...
//
// Not file-local: exported (see atmos.hpp) so tests/performance/ac3kernelbench
// can bench it in isolation, the same reason ecpl_channel_spectrum and the
// AHT tools are exported from eac3_tools.hpp despite not being part of the
// main public encode/decode API.
...
std::expected<eac3::AccessUnit, FrameError> AtmosEncoder::encode_frame(
    ...
    AC3_ZONE_BEGIN(zone_bed, "bed_render");
    ...
    AC3_ZONE_BEGIN(zone_joc_matrix, "joc_covariance_invert");
