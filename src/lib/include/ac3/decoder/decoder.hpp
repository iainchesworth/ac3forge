...
// and 2/0 rematrixing. acmod 0 (1+1 dual mono) is two independent
// programmes sharing one syncframe — Ch2's dialnorm2/compr2/dynrng2 are
// parsed and reported alongside Ch1's, and each programme's §7.7 gain is
// applied to its own channel only. Deliberately unsupported (clean errors,
// not wrong audio): block switching, delta bit allocation. dynrng words are
