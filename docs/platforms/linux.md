...
Built and tested with **GCC 15.2** and **Clang 21.1** on **Ubuntu 26.04 (WSL2)** — GCC ≥ 15 or
Clang ≥ 21 is required.

`config-linux-gcc` and `config-linux-llvm` (each with a `-debug` variant) find the compiler by
an exact-version `find_program` the same way the Windows presets pin MSVC/clang-cl, rather than
...
status** travelling beside them (the non-audio bit, AES0 bit 1). ALSA is where that bit is
expressed, as arguments on the device name (`iec958:CARD=PCH,DEV=0,AES0=0x06,…`). PulseAudio's
`PA_STREAM_PASSTHROUGH` and PipeWire's `SPA_MEDIA_SUBTYPE_iec958` are both real, but both end in
the same ALSA call made by a daemon instead of by us — so ALSA isn't merely the lowest common
denominator, it's the layer the other two are built on, present on every Linux system including
ones running no sound server at all. The cost is coexistence: opening a device directly takes it
exclusively, so a running sound server has to have released it first — the same bargain WASAPI
exclusive mode strikes on Windows, for the same reason (a mixer that resamples or
volume-scales a burst stream turns it into noise).
...
it supplies Catch2 and nothing else, same as on Windows; this project's own convention keeps it
...
(or `pack-linux-llvm` for the Clang build). Produces a plain tarball, plus DEB/RPM on top when
the corresponding packaging tool is on `PATH`. Only the Windows packaging legs have actually
been run to date — the Linux `pack-*` presets exist and are wired up, but Linux Qt packaging
doesn't apply yet since `AC3FORGE_BUILD_GUI` still defaults off on every non-Windows preset
even though CI now builds and smoke-tests the GUI on both Linux legs. See
...
The ALSA backend adds 14 tests of its own (`tests/platform/alsa/`): the suite is 256 ctest
entries on a Linux build without `libasound2-dev`, the same count as Windows, and 270 with ALSA
