...
linux-llvm, linux-llvm-asan-ubsan, static-analysis (clang-tidy) and coverage (gcovr over
`src/lib`, via `config-linux-gcc-coverage`) on every push — the two Linux legs install the same
Qt6/ALSA packages and build/smoke-test the GUI too. Only macos-llvm remains experimental
(`continue-on-error`).

The coverage job's own gate — 83.5% line / 74.6% branch measured, 80%/70% required — was run the
