...
| | Version | Notes |
|---|---|---|
| A compiler | MSVC (VS 2026), clang-cl 21, GCC 15, or Clang 21 | C++23. One preset per compiler. |
| CMake | ≥ 3.28 | |
| Ninja | any recent | The presets hard-code the Ninja generator. |
| [vcpkg](https://github.com/microsoft/vcpkg) | any recent, with `VCPKG_ROOT` set | Supplies Catch2, and nothing else. |
...
`ctest` runs the full suite (249 Catch2 unit tests plus the example programs). See
...
- [CLI reference](cli/index.md) — `ac3cli`, the twenty-one-command front end, for encoding and
